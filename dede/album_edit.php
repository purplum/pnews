...
else if($dopost=='save')
{
    ...
    for($i=1; $i<=120; $i++)
    {
		...
		$idesc = str_replace("'", "`", stripslashes(${'imgdesc'.$i}));
		$ilink1 = str_replace("'", "`", stripslashes(${'imglink1'.$i}));
			$ilink2 = str_replace("'", "`", stripslashes(${'imglink2'.$i}));
        ...
        if( isset(${'imgfile'.$i}) && is_uploaded_file(${'imgfile'.$i}) )
        {
            ...
            if(!is_array($imginfos))
            {
                ...
                $imgurls .= "{dede:img ddimg='$ddurl' text='$iinfo' desc='$idesc' link1='$ilink1' link2='$ilink2' width='".$imginfos[0]."' height='".$imginfos[1]."'} $iurl {/dede:img}\r\n";
                continue;
            }
