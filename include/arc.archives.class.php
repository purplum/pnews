...
$positionArr = explode($GLOBALS['cfg_list_symbol'], $this->TypeLink->GetPositionName()); 
$this->Fields['wumiicate'] = $positionArr[0];
            {
...
if($ismake=1) 
{ 
$body=preg_replace("/<(img|IMG)(.*)(src|SRC)=[\"|'| ]{0,}((.*)>)/isU","<br/><a href='".$this->NameFirst."_".$lPage.".".$this->ShortName."'>"."\${0}"."</a>",$body); 
} 
else 
{ 
...
return $body; 
    ...
    function ParseDMFields($pageNo, $ismake=1)
    {
        ...
        if($this->SplitPageField!='' && isset($this->Fields[$this->SplitPageField]))
        {
           ...
           //分页文章摘要一致 if($pageNo>1) $this->Fields['description'] = trim(preg_replace("/[\r\n\t]/", ' ', cn_substr(html2text($this->Fields[$this->SplitPageField]), 200)));
        }
    ...
    function GetPagebreakDM($totalPage, $nowPage, $aid)
    {
        ...
        else
        {
            ...
            else
            {
				...
				$total_list=9; //显示7个，3*2+1=7 
if($nowPage >= $total_list) { 
$i = $nowPage-4; //前后6个，（7-1）/ 2=3 
$total_list = $nowPage+4; //前后3个，（7-1）/ 2=3 
...
$total_list = $totalPage; 
...

        {
            if($i==1)
            {
                ...
                {
                    $PageList.="<span class=\"thisclass\"><a>1</a></span>";
                }
            }
            else
            {
                ...
                {
                    $PageList.="<span class=\"thisclass\"><a href='#'>{$n}</a></span>";
                }
            }
        }
        if($lPage <= $totalPage)
...
$total_list = $totalPage; 
        ...
        for($i;$i<=$total_list;$i++)
        {
            if($i==1)
            {
                ...
                {
                    $PageList.="<span class=\"thisclass\"><a href='#'>1</a></span>";
                }
            }
            else
            {
