...
var
    cMediaPlayerTypes = ['application/x-mplayer2', 'audio/x-pn-realaudio-plugin', 'video/quicktime', 'application/x-shockwave-flash', 'application/x-director', 'video/qvodplayer']
  , cMediaPlayerClassId = [cWMp6Compatible? 'clsid:05589FA1-C356-11CE-BF01-00AA0055595A' : 'clsid:6BF52A52-394A-11D3-B153-00C04F79FAA6', 'clsid:CFCDAA03-8BE4-11cf-B84B-0020AFBBCCFA'
        ...
        , 'clsid:166B1BCA-3F9C-11CF-8075-444553540000', 'clsid:F3D0D36F-23F8-4682-A195-74C92B03D4AF'
    ]
...
var
  ...
  oQvodPlayer = {id: cQvodPlayer, type: cMediaPlayerTypes[cQvodPlayer], ClsId: cMediaPlayerClassId[cQvodPlayer]
    , Params: {autoplay:1, showcontrol :1
      }
  };

var
  oFCKMediaPlayers = [oWindowMediaPlayer, oRealMediaPlayer, oQuickTimePlayer, oFlashPlayer, oShockwavePlayer, oQvodPlayer];
...
function WrapObjectToMedia(html, aMediaElementName) {
  ...
  function _ConvertMedia( m, params )
  {
    ...
    for (var i = 0; i < params.length; i++) {
      ...
      if (vParamName == cMediaTypeAttrName) {
        if (isInt(vParamValue)) {
          vIsMedia = true;
          if(vParamValue == '5') vIsQvod = true;
          vObjectAttrs += ' '+ cMediaTypeAttrName + '="' + vParamValue + '"';
          vObjectAttrs += ' classid="' +  oFCKMediaPlayers[vParamValue].ClsId + '"';
          if(oFCKMediaPlayers[vParamValue].Codebase) vObjectAttrs += ' codebase="' +  oFCKMediaPlayers[vParamValue].Codebase + '"';
        }else {
          break;
        }
    ...
    if (vIsMedia) {
      Result = '<object' + vObjectAttrs + '>' + Result ;
      if(!vIsQvod){	Result += '<embed' + m + '></embed>' }
      Result += '</object>';
      //alert(Result);
      return Result;
    }
